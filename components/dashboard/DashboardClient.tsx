...
export default function DashboardClient({ stats, recentAlerts }: Props) {
  ...
  const stats4 = [
    { label: 'EXPIRED',        value: String(stats.expired),      urgent: stats.expired > 0 },
    { label: 'EXPIRING SOON',  value: String(stats.expiringSoon), urgent: false },
    { label: 'COMPLIANT',      value: String(stats.valid),        urgent: false },
    { label: 'PENDING REVIEW', value: String(stats.pendingReview),urgent: false },
  ];
      ...
      <div className="card-flush">
        ...
        <div className="p-6">
          <div className="grid grid-cols-10 gap-2">
            {[...Array(30)].map((_, i) => {
              // Mark days that have expiring alerts
              const dayDate = new Date(Date.now() + i * 24 * 60 * 60 * 1000).toISOString().split('T')[0];
              const hasAlert = recentAlerts.some(a => a.expiryDate === dayDate);
              const isCritical = recentAlerts.some(a => a.expiryDate === dayDate && a.severity === 'critical');
              const day = i + 1;
              return (
